...
static int noise_curve25519_set_keypair(NoiseDHState* state,
    ...
    EVP_PKEY* private_key_ctx = EVP_PKEY_new_raw_private_key(
	EVP_PKEY_X25519, NULL, private_key, LOCAL_KEY_SIZE);
    EVP_PKEY_get_raw_public_key(private_key_ctx, public_key_tmp,
				&public_key_tmp_len);
    EVP_PKEY_free(private_key_ctx);

    equal =
	noise_is_equal(public_key_tmp, public_key, st->parent.public_key_len);
...
static int noise_curve25519_set_keypair_private(NoiseDHState* state,
    ...
    EVP_PKEY* private_key_ctx = EVP_PKEY_new_raw_private_key(
	EVP_PKEY_X25519, NULL, private_key, LOCAL_KEY_SIZE);
    EVP_PKEY_get_raw_public_key(private_key_ctx, st->parent.public_key,
				&public_key_len);
    EVP_PKEY_free(private_key_ctx);

    if (public_key_len != LOCAL_KEY_SIZE) return NOISE_ERROR_INVALID_LENGTH;
...
static int noise_curve25519_calculate(const NoiseDHState* private_key_state,
				      ...
				      uint8_t* shared_key) {
    return X25519(shared_key, private_key_state->private_key,
...
NoiseDHState* noise_curve25519_new(void) {
    ...
    state->parent.shared_secret_key_len = X25519_KEY_LENGTH;
